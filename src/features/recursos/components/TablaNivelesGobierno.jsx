...
const TablaNivelesGobierno = ({ubigeo=0, instrumento='',departamento=''}) => {
  ...
  const getResumen = async () =>{
    try {
        setLoading(true);
        await axiosClient.get(`/niveles_gobierno/${ubigeo}/${instrumento}`)
            .then(({data}) => {
                setData(data.data);
                setLoading(false);
        })
 ...
 getResumen();
  ...
  return (
    ...
    <div className="max-w-5xl p-2 mx-auto fill-gray-400">
        <h1 className="mb-4 text-lg font-bold text-center">{instrumento} POR NIVELES DE GOBIERNO {departamento}</h1> 
