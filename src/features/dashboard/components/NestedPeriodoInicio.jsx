...
const getResumen = async (ub) => {
  const {data} = await axiosClient.get(`/periodos_pprrd_inicio/${ub}`);
          return data;
  } 
...
const NestedPeriodoInicio = ({ubigeo=0}) => {
    ...
    if(!isLoadingfort){
        ...
        var totalespecializada=df.data[0].espcializada
        var totalgeneral=df.data[0].total
        
    }


  const {data,error,isLoading} = useQuery(["nestedPeriodort"+ubigeo], () => getResumen(ubigeo));
  ...
  const distritos =!isLoading ? data[0].distritos:0;
  var total_evaluadores = (!isLoading ) ? data[0].total_evaluadores:0;


  var total_evar = !isLoading ? data[0].total_evar:0;
  var totalentidades =!isLoading ? data[0].total_entidades:0;
  
  const totalsesiones = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.sesiones) || 0), 0) : 0;
  var total_basica =!isLoading ? data[0].total_formacion_basica:0;
  var total_especializada =!isLoading ? data[0].total_formacion_especializada:0;
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M12 16.75C12.2106 16.75 12.4114 16.6615 12.5535 16.5061L16.5535 12.1311C16.833 11.8254 16.8118 11.351 16.5061 11.0715C16.2004 10.792 15.726 10.8132 15.4465 11.1189L12.75 14.0682V3C12.75 2.58579 12.4142 2.25 12 2.25C11.5858 2.25 11.25 2.58579 11.25 3V14.0682L8.55353 11.1189C8.27403 10.8132 7.79963 10.792 7.49393 11.0715C7.18823 11.351 7.16698 11.8254 7.44648 12.1311L11.4465 16.5061C11.5886 16.6615 11.7894 16.75 12 16.75Z" fill="#FFFFFF"/>
          ...
          table="periodosinicio"
          filename="periodos_resumen"
          sheet="Resumen"
          buttonText="Excel"
          />  
          
    </div>
      <table id="periodosinicio" className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="cabecera-cenepred">
            <th rowSpan={2} className="border border-gray-300 p-2">PERIODOS</th>
            <th colSpan={3} className="border border-gray-300 p-2">ASISTENCIA TÉCNICA PPRRD</th>
            ...
            <th rowSpan={2} className="border border-gray-300 p-2">EVAR ACREDITADO</th>
          ...
          <tr className="text-center">
            ...
            <th className="text-xs border border-gray-300">PPRRD APROBADOS</th>
          ...
          {isLoading ? (
            ...
            data?.map((dept, deptIndex) => (
               ...
               <React.Fragment key={dept.periodo}>
                <tr key={dept.periodo} className="hover:bg-gray-200 text-center">
                  ...
                  <td className="border border-gray-300 p-2">{Number(dept.distritos)-Number(dept.pprrd)}</td>
                  <td className="border border-gray-300 p-2">{dept.distritos}</td>
        ...
        {<tr className="font-bold bg-gray-100">
              ...
              <td className="border text-center border-gray-300 p-2">{distritos}</td>
              <td className="border text-center border-gray-300 p-2">{totalpprrd_vigentes}</td>
              <td className="border text-center border-gray-300 p-2">{distritos-totalpprrd_vigentes}</td>
              <td className="border text-center border-gray-300 p-2">{distritos}</td>
