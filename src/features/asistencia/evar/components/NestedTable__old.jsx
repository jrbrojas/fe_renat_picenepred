...
useEffect(() => {
  ...
  const getResumen = async () =>{
    setLoading(true)
    try {
        await axiosClient.get(`/departamen_detalle/${ubigeo}`)
            .then(({data}) => {
                setData(data);
                setLoading(false);
    ...
    <table className="w-full border-collapse border border-gray-300">
      <thead>
        <tr className="cabecera-cenepred">
          ...
          <th className="border border-gray-300 p-2">EEVAR APROBADOS</th>
          <th className="border border-gray-300 p-2">EVAR VIGENTES</th>
