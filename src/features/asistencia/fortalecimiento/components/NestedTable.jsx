...
const getResumen = async (u,i) => {
  const {data} = await axiosClient.get(`/fortalecimiento_cursos/${u}`);
          return data;
  } 

const NestedTable = ({ubigeo=0}) => {

/*const { data:dr,isLoading: isLoadingConteo } = useResumen(ubigeo, 'fortalecimiento_conteo');
if(!isLoadingConteo){
    //var totalbasica=dr.data[0].basica
    //var totalespecializada=dr.data[0].especializada
    //var totalgeneral=dr.data[0].total
   // const totalbasica = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.total_basica) || 0), 0) : 0;
    }
*/
  const {data,error,isLoading} = useQuery(["NestedTableFort"+ubigeo], () => getResumen(ubigeo));

  const totalbasica = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.total_basica) || 0), 0) : 0;
  const totalespecializada = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.total_especializada) || 0), 0) : 0;
  ...
  //const totalbasica = !isLoading ? data.reduce((sum, d) => sum + (parseInt(d.basica) || 0), 0) : 0;
...
<div className="btn-excel-cenepred" style={{display:'flex',width:'75px'}}>
    ...
    <path fill-rule="evenodd" clip-rule="evenodd" d="M12 16.75C12.2106 16.75 12.4114 16.6615 12.5535 16.5061L16.5535 12.1311C16.833 11.8254 16.8118 11.351 16.5061 11.0715C16.2004 10.792 15.726 10.8132 15.4465 11.1189L12.75 14.0682V3C12.75 2.58579 12.4142 2.25 12 2.25C11.5858 2.25 11.25 2.58579 11.25 3V14.0682L8.55353 11.1189C8.27403 10.8132 7.79963 10.792 7.49393 11.0715C7.18823 11.351 7.16698 11.8254 7.44648 12.1311L11.4465 16.5061C11.5886 16.6615 11.7894 16.75 12 16.75Z" fill="#FFFFFF"/>
              ...
              filename="departamen_capacitados"
              sheet="Resumen"
              buttonText="Excel"
              />  
    ...
    <table id="tablacapacita" className="w-full border-collapse border border-gray-300">
      <thead>
        <tr className="bg-gray-200 cabecera-cenepred">
          <th className="border border-gray-300 p-2">DEPARTAMENTO</th>
          <th className="border border-gray-300 p-2">FORMACIÓN<br/>BÁSICA</th>
          <th className="border border-gray-300 p-2">FORMACIÓN<br/>ESPECIALIZADA</th>
 ...
 <tbody>
        {data.map((dept, deptIndex) => (
           <React.Fragment key={deptIndex}>
            ...
            >
              ...
              <td className="border border-gray-300 p-2">{dept.total_basica}</td>
              <td className="border border-gray-300 p-2">{dept.total_especializada}</td>
              <td className="border border-gray-300 p-2">{dept.total}</td>
              </tr>
              ...
              dept.provincias.map((prov, provIndex) => (
                <React.Fragment key={provIndex}>
                  ...
                  >
                    ...
                    <td className="border border-gray-300 p-2">{prov.total_basica}</td> 
                    <td className="border border-gray-300 p-2">{prov.total_especializada}</td> 
                  ...
                  {expandedProvinces[`${deptIndex}-${provIndex}`] &&
                    prov.distritos.map((dist, distIndex) => (
                      <tr key={`${dept.id}-${prov.id}-${dist.id}`}>
                        <td className="border border-gray-300 p-2 pl-12"> - {dist.distrito}</td>
                        <td className="border border-gray-300 p-2">{dist.total_basica}</td>
                        <td className="border border-gray-300 p-2">{dist.total_especializada}</td>
     ...
     {<tr className="font-bold bg-gray-100">
              ...
              <td className="border border-gray-300 p-2">{totalbasica}</td>
              <td className="border border-gray-300 p-2">{totalespecializada}</td>
              <td className="border border-gray-300 p-2">{totalgeneral}</td>
