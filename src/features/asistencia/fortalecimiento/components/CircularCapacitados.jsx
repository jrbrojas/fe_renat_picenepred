...
const CircularCapacitados = ({ nivel='basica', size = 300,title='',ubigeo=0}) => {
  ...
  useEffect(() => {
    const getResumen = async () =>{
      try {
               ...
               await axiosClient.get(`porcentaje_capacitados/${ubigeo}/${nivel}`)
              .then(({data}) => {
                
                const {dato}= data.data[0];
                console.log(dato);
  ...
  return (
    <>
     <h1 className="mb-4 mt-4  text-lg font-bold text-center">{title}</h1>
    <div className="mt-4 mx-auto relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute text-3xl font-bold">{avance}%</div>
    </div>
    </>
  );
};

export default CircularCapacitados
