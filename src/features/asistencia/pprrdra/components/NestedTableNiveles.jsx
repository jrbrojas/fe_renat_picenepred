...
const getResumen = async (u,i) => {
  const {data} = await axiosClient.get(`/niveles_detalle/${u}/${i}`);
          return data;
  } 


const NestedTableNiveles = ({ubigeo=0,instrumento=''}) => {

  
const {data,error,isLoading} = useQuery(["nestNiveles_gob"+ubigeo+"-"+instrumento], () => getResumen(ubigeo,instrumento));
  ...
  const totalnacional = !isLoading ? data.reduce((sum, d) => sum + (parseInt(d.nacional) || 0), 0) : 0;
  const totalregional= !isLoading ? data.reduce((sum, d) => sum + (parseInt(d.regional) || 0), 0) : 0;
  const totalprovincial= !isLoading ? data.reduce((sum, d) => sum + (parseInt(d.provincial) || 0), 0) : 0;
  const totaldistrital = !isLoading ? data.reduce((sum, d) => sum + (parseInt(d.distrital) || 0), 0) : 0;
  ...
  return (
    ...
    <table id="tabla_niveles" className="w-full border-collapse border border-gray-300">
      <thead>
        <tr className="cabecera-cenepred">
          <th className="border border-gray-300 p-2">DEPARTAMENTO</th>
          <th className="border border-gray-300 p-2">NACIONAL</th>
          <th className="border border-gray-300 p-2">REGIONAL</th>
          <th className="border border-gray-300 p-2">PROVINCIAL</th>
          <th className="border border-gray-300 p-2">DISTRITAL</th>
 ...
 <tbody>
        {data.map((dept, deptIndex) => (
          <React.Fragment key={dept.ubigeo_departamento}>
            <tr
              key={dept.ubigeo_departamento}
            ...
            >
              ...
              <td className="border border-gray-300 p-2">{dept.nacional}</td>
              <td className="border border-gray-300 p-2">{dept.regional}</td>
              <td className="border border-gray-300 p-2">{dept.provincial}</td>
              <td className="border border-gray-300 p-2">{dept.distrital}</td>
              <td className="border border-gray-300 p-2">{dept.total}</td>
              </tr>
            {expandedDepartments[deptIndex] &&
              dept.provincias.map((prov, provIndex) => (
                <React.Fragment key={prov.ubigeo_provincia}>
                  <tr
       ...
       {<tr className="font-bold">
        ...
        <td className="border border-gray-300 p-2">{totalnacional}</td>
        <td className="border border-gray-300 p-2">{totalregional}</td>
        <td className="border border-gray-300 p-2">{totalprovincial}</td>
        <td className="border border-gray-300 p-2">{totaldistrital}</td>
...
export default NestedTableNiveles;
