...
const getResumen = async (ub) => {
  const {data} = await axiosClient.get(`at_departamentos_otros/${ub}`);
          return data;
  } 


const NestedDepartamentoInicio = ({ubigeo=0}) => {

  const {data,error,isLoading} = useQuery(["nestedDepartamentosW"+ubigeo], () => getResumen(ubigeo));


  const total_pec = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.pec) || 0), 0) : 0;
  var total_reaas = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.reas_pob) || 0), 0) : 0;
  var total_otros = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.otros) || 0), 0) : 0;
  ...
  var total_entidades = (!isLoading ) ? data.reduce((sum, d) => sum + (parseInt(d.entidades_asistidas) || 0), 0) : 0;
...
/*
 ...
 <div className="btn-excel-cenepred" style={{display:'flex',width:'75px'}}>
                          ...
                          <path fill-rule="evenodd" clip-rule="evenodd" d="M12 16.75C12.2106 16.75 12.4114 16.6615 12.5535 16.5061L16.5535 12.1311C16.833 11.8254 16.8118 11.351 16.5061 11.0715C16.2004 10.792 15.726 10.8132 15.4465 11.1189L12.75 14.0682V3C12.75 2.58579 12.4142 2.25 12 2.25C11.5858 2.25 11.25 2.58579 11.25 3V14.0682L8.55353 11.1189C8.27403 10.8132 7.79963 10.792 7.49393 11.0715C7.18823 11.351 7.16698 11.8254 7.44648 12.1311L11.4465 16.5061C11.5886 16.6615 11.7894 16.75 12 16.75Z" fill="#FFFFFF"/>
                                    ...
                                    table="otros_departamentos"
                                    filename="otros_departamentos"
                                    sheet="Resumen"
                                    buttonText="Excel"
                                    />  
                              </div>
      <table id="otros_departamentos" className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="cabecera-cenepred">
            <th rowSpan={2} className="border border-gray-300 p-2">DEPARTAMENTO</th>
          ...
          {isLoading ? (
            ...
            data?.map((dept, deptIndex) => (
               ...
               <React.Fragment key={dept.periodo}>
                <tr key={`${dept.periodo}-${deptIndex}`}  className="hover:bg-gray-200">
                  <td className="border border-gray-300 p-2">{dept.dpto}</td>
                  <td className="border text-center  border-gray-300 p-2">{dept.pec}</td>
                  <td className="border text-center border-gray-300 p-2">{dept.reas_pob}</td>
                  <td className="border text-center border-gray-300 p-2">{dept.otros}</td>
                  <td className="border text-center  border-gray-300 p-2">{dept.act_rep}</td>
                  <td className="border text-center border-gray-300 p-2">{parseInt(dept.pec)+parseInt(dept.reas_pob)+parseInt(dept.otros)+parseInt(dept.act_rep)}</td>
                  <td className="border text-center border-gray-300 p-2">{dept.total_aprobados}</td>
                  <td className="border text-center border-gray-300 p-2">{dept.entidades_asistidas}</td>   
                </tr>
              </React.Fragment>
        ...
        {<tr className="font-bold bg-gray-100">
              ...
              <td className="border text-center border-gray-300 p-2">{total_reaas}</td>
              ...
              <td className="border text-center border-gray-300 p-2">{total_pec+total_reaas+total_otros+total_act}</td>
...
export default NestedDepartamentoInicio;
