...
const DashboardTableResumenpprrd =({ubigeo,instrumento,titulo=''}) => {

  const {data:d,error,isLoading} = useQuery(["periodos_inst"+ubigeo+instrumento], () => getResumen(ubigeo,instrumento));
  
  if(!isLoading && d.length>0){
  var totalentidades = d[0].total_entidades;
  }
    else{
        var totalentidades=0;
    }
  ...
  return (
    ...
    <div className="max-w-5xl p-2 mx-auto fill-gray-400">
       ...
       <div className="btn-excel-cenepred" style={{display:'flex',width:'75px'}}>
        ...
        <table id="paa_periodos" className="w-full border-collapse border border-gray-300">
