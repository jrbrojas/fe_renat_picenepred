...
const {data,error,isLoading} = useQuery(["nestPPRRD1"+ubigeo+instrumento], () => getResumen(ubigeo,instrumento));
//console.log(data[0].total_entidades);
  ...
  const totalbrecha = !isLoading ? data.reduce((sum, d) => sum + (parseInt(d.distritos)-parseInt(d.pprrd_vigentes) || 0), 0) : 0;
  ...
  return (
    ...
    isLoading ? (
      ...
      <div className="p-1">
            ...
            <path fill-rule="evenodd" clip-rule="evenodd" d="M12 16.75C12.2106 16.75 12.4114 16.6615 12.5535 16.5061L16.5535 12.1311C16.833 11.8254 16.8118 11.351 16.5061 11.0715C16.2004 10.792 15.726 10.8132 15.4465 11.1189L12.75 14.0682V3C12.75 2.58579 12.4142 2.25 12 2.25C11.5858 2.25 11.25 2.58579 11.25 3V14.0682L8.55353 11.1189C8.27403 10.8132 7.79963 10.792 7.49393 11.0715C7.18823 11.351 7.16698 11.8254 7.44648 12.1311L11.4465 16.5061C11.5886 16.6615 11.7894 16.75 12 16.75Z" fill="#FFFFFF"/>
                      ...
                      table="departamento_pprrd"
                      filename="departamento_pprrd"
                      sheet="Resumen"
                      buttonText="Excel"
                      />  
    ...
    <table id="departamento_pprrd" className="w-full border-collapse border rounded-lg border-gray-300">
      <thead>
        <tr className="cabecera-cenepred">
          ...
          <th className="border border-gray-300 p-2">ENTIDADES <br/>ASISTIDAS</th>
          <th className="border border-gray-300 p-2">{instrumento} <br/>APROBADOS</th>
           <th className="border border-gray-300 p-2">{instrumento} VIGENTES</th>
           {(instrumento!='EVAR') ?<th className="border border-gray-300 p-2">BRECHA</th>:''}
           <th className="border border-gray-300 p-2">SESIONES</th>
        </tr>
      </thead>
 <tbody>
        {data.map((dept, deptIndex) => (
          <React.Fragment key={dept.departamentos_id}>
            ...
            >
              ...
              <td className="border border-gray-300 p-2">{dept.total_entidades}</td>
              <td className="border border-gray-300 p-2">{dept.pprrd}</td>
              <td className="border border-gray-300 p-2">{dept.pprrd_vigentes}</td>
              {(instrumento!='EVAR') ? <td className="border border-gray-300 p-2">{Number(dept.distritos)-Number(dept.pprrd_vigentes)}</td>:''}
                ...
                <React.Fragment key={prov.provincias_id}>
                  ...
                  >
                    <td className="border border-gray-300 p-2 pl-6 flex items-center gap-2">
                      {expandedProvinces[`${deptIndex}-${provIndex}`] ? <ChevronDown size={18} className="icono-cenepred" /> : <ChevronRight size={18} className="icono-cenepred"/>}
                      {prov.provincia}
                    </td>
                   ...
                   { (instrumento!='EVAR') ? <td className="border border-gray-300 p-2">{Number(prov.distritos)-Number(prov.pprrd_vigentes)}</td>:''}
                   <td className="border border-gray-300 p-2">{prov.sesiones}</td>
                  ...
                  {expandedProvinces[`${deptIndex}-${provIndex}`] &&
                    prov.distrito.map((dist, distIndex) => (
                      <tr key={`${dept.departamentos_id}-${prov.provincias_id}-${dist.distritos_id}`}>
                        ...
                        {(instrumento!='EVAR') ?  <td className="border border-gray-300 p-2">-</td>:''}
                        <td className="border border-gray-300 p-2">{dist.sesiones}</td>
      ...
      { <tr className="font-bold">
        ...
        {(instrumento!='EVAR') ? <td className="border border-gray-300 p-2">{totalbrecha}</td>:''}
        <td className="border border-gray-300 p-2">{totalsesiones}</td>
      </tr>}
     </tbody>
    </table>
  </div>
